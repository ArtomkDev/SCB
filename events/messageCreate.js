...
module.exports = {
    ...
    async execute(message, client) {
        ...
        if (isMentioned || isRandomReply) {
            ...
            activityContext += "\n[КРИТИЧНА СИСТЕМНА ВКАЗІВКА ЩОДО АКТИВНОСТІ]: Ти бачиш поточну активність користувачів, але ТОБІ СУВОРО ЗАБОРОНЕНО згадувати те, чим вони займаються, у кожному повідомленні. Згадуй про їхні ігри чи програми ТІЛЬКИ тоді, коли користувач сам про це запитав, або це ідеально лягає в контекст (максимум 1 раз на всю розмову). Якщо користувач довго сидить у програмі (наприклад, Visual Studio Code), просто ігноруй це. Якщо все ж доводиться сказати про час, НІКОЛИ не називай точні цифри, навіть округлені (ніяких '20 хвилин' чи 'півгодини'). Говори як звичайна людина: 'давненько', 'якийсь час', 'щойно'. Будь природним і не будь нав'язливим!\n---------------------------------------\n";
            ...
            conversationContext += `\n\n[АБСОЛЮТНИЙ ПРІОРИТЕТ - ОЦІНКА СИТУАЦІЇ]:`;
            conversationContext += `\nЗараз користувач ${message.author.displayName} написав тобі: "${message.content}".`;
            conversationContext += `\nПРАВИЛО 1 (Нормальна розмова): Якщо користувач задає нормальне питання, просто спілкується, або щось просить (наприклад "скинь гіфку", "що робиш", "як справи") — відповідай НОРМАЛЬНО, розгорнуто, підтримуй діалог у своєму фірмовому стилі.`;
            conversationContext += `\nПРАВИЛО 2 (Спам і тупі повторення): ТІЛЬКИ ЯКЩО ти бачиш, що людина відверто спамить і повторює одне й те саме по колу (наприклад, пише "ще раз", "і що" кілька разів підряд) — зламай шаблон! Вимкни капс і відповідай максимально коротко і зневажливо (1-3 слова). Приклади: "від'їбись", "завали", "клоун".`;
            conversationContext += `\nТвоє ім'я: ${client.user.username}. Оціни ситуацію і напиши відповідь:`;
